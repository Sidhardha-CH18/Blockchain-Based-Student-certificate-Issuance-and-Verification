    ...
    <nav className={`w-full py-4 px-6 transition-all duration-300 ${scrolled ? 'bg-slate-900/90 backdrop-blur-md shadow-md' : 'bg-transparent'}`}>
      <div className="container mx-auto flex justify-between items-center">
        ...
        <div className="hidden md:flex items-center space-x-8">
          ...
          <NavLink href="/signup" isButton={true}> {/* Update here */}
            Sign Up
          </NavLink>
        </div>
      </div>
...
const NavLink = ({ href, children, className = '', isButton = false }) => {
  return (
    <a href={href} className={` 
        ${isButton ? 
          'bg-gradient-to-r from-purple-500 to-pink-500 text-white px-5 py-2 rounded-full font-medium shadow-lg shadow-purple-600/30 hover:shadow-xl hover:scale-100 transition-all transform' 
           : 
          'text-gray-200 hover:text-white relative after:absolute after:bottom-0 after:left-0 after:h-0.5 after:w-0 hover:after:w-full hover:after:bg-gradient-to-r hover:after:from-red-400 hover:after:to-blue-400 after:transition-all'}
        ${className}
      `}>
      {children}
    </a>
